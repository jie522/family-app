...
這個 repo 同時是「家庭小站」App(追劇清單 + 台股追蹤)的原始碼,也是這份個人知識庫。兩者關係:
...
App 的股票詳情頁用「公司簡介/產業知識/分析報告」**三個頁籤**呈現,所以**第一次幫某檔股票建報告時,三種檔案都要一起建好**,缺一個該頁籤就會空著:
...
之後的例行更新以「新增日期報告 + 更新 manifest」為主;`_about.md` / `_industry.md` 只在內容過時或有新資訊時編輯補充。
