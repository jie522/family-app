...
App 底部有個「📚 知識庫」頁籤,收錄產業知識與投資框架的公開短文(如晶圓代工、封裝與測試、AI供應鏈等),彼此可以互相連結、也能連去對應的股票詳情頁。內容放在 `knowledge/` 資料夾:

- `knowledge/manifest.json` — 索引清單,格式 `[{"slug","title","summary"}, ...]`
- `knowledge/<slug>.md` — 內文,支援站內連結:`[文字](k:另一篇slug)` 跳轉知識庫文章、`[文字](s:股票代號)` 跳轉股票詳情頁

新增一篇知識庫文章,記得同時把它加進 `manifest.json`,不然不會出現在列表裡。
